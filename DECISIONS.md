...
- **iOS 26 Liquid Glass on chrome only** — nav, sheets, island. Content cards stay solid so hierarchy holds (Apple HIG: glass for controls, not content). No tab bar.
...
- **No tab bar. The desk is the app** — second Jobs/Bezos pass. Three tabs still made Names and Record compete with the only job. Home is a state machine: score a print if one is in, else lock a call on the next one (Up / Down / Flat live on the card — no “Write the call” sheet for the main path). Names and Record are push screens from search and the accuracy chip. Retrieve the three-tab layout at git tag `three-tabs` (also `consumer-redesign`).
