...
- **News tab is a tape, not a clipper** — Yahoo-style watchlist feed with impact flags. High (red) ≈ >2% next-session, Med (amber) 0.5–2%, Low is color. Heuristic first so the screen paints without a model call; **Rank with Grok** is user-initiated and capped at 12 headlines. Stories group by name. High stories also sit on Now.
